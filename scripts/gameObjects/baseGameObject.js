...
//BaseGameObject is from Lukaes Template
...
class BaseGameObject {
  ...
  applyGravity = function () {
    ...
    for (let i = 0; i < global.allGameObjects.length; i++) {
      ...
      if (
        ...
        otherObject.blockGravityForces == true
      ) {
  ...
  getNextSprite = function () {
    ...
    if (
      this.animationData.currentSpriteElapsedTime >=
      this.animationData.timePerSprite
    ) {
      this.animationData.currentSpriteIndex++;
      this.animationData.currentSpriteElapsedTime = 0;
      if (
        this.animationData.currentSpriteIndex >
        this.animationData.lastSpriteIndex
      ) {
        this.animationData.currentSpriteIndex =
          this.animationData.firstSpriteIndex;
      }
    }
    return this.animationData.animationSprites[
      this.animationData.currentSpriteIndex
    ];
  };
  ...
  loadImagesFromSpritesheet(spritesheetPath, cols, rows) {
    ...
    this.animationData.animationSprites = Array.from(
      { length: totalSprites },
      () => new Image()
    );
